            ...
            if (this.shouldPrebuildIncrementally(context.repository.cloneUrl, project)) {
                ...
                if (repoProvider) {
                    ...
                    if (context.additionalRepositoryCheckoutInfo && context.additionalRepositoryCheckoutInfo.length > 0) {
                        const histories = context.additionalRepositoryCheckoutInfo.map(info => repoProvider.getCommitHistory(user, info.repository.owner, info.repository.name, info.revision, maxDepth));
